...
The specific goal of our project is to create a smartphone application which classifies user inputted pictures of mushrooms to edible or not edible, using a machine learning classifier such as a convolutional neural network (CNN). CNN:s are the current state-of-the-art in image recognition. 

To train such a classifier, we'll need large amounts of pictures of previously classified mushrooms to use as examples. We cannot simply use any pictures of mushrooms, since they have to include a reliable edibility tag. The name of mushroom would be enough though, since the edibility status can be retrived based on the name. We'll also have to normalize the size and shape of any pictures we'll find, since usually classification algorithms expect prespecified input sizes.

As already mentioned, we'll have to pay close attention to the possibility of false edibles. Instead of simply outputting a binary edible or not edible status, we'll instead provide the user with the *probability* of having an edible mushroom in their hands. If the user finds that the probability of edibility is high, the user can decide to collect the mushroom and do further reseach at home. Our app is therefore a tool for deciding whether to collect a mushroom or not, not a tool for decising whether to eat the mushroom or not.
...
In addition to the possibilities listed above, there are also features such as smell and touch that could be useful. However, we do not expect to be able to explicitly extract these features. However, a CNN performs feature extraction implicitly based on an input image. It is "simply" a matter of defining the structure of the network in such a way that the network is able to extract the information it needs for the classification task.

# Retrieving and transforming data

www.mushroom.world contains a large database of mushroom pictures and information such as edibility status. We build a web scraped which retrieves images and their information from the website. From these mushrooms we have collected, besides the edibility, their names in Finnish, English and Latin. We also collected the URL:s for the available images of the mushshroom and then downloaded the images.  

In mushroom.world, the edibility was originally classified into 7 different classes: "edible", "edible and good", "edible and excellent", "not edible", "edible when cooked", "poisonous", "leathally poisonous". We decided to treat the problem as a binary classification task and narrowed the edibility status down to edible or non-edible. We categorized "edible when cooked" as non-edible, which means that all edible mushrooms require no preprocessing and should be safe to eat. 

The images and the metadata are stored in both Google Drive and Amazon S3. The original pictures were very good quality and therefore took a lot of space. We used .jpg compression and resized the images. In the original set of images, the sizes varied quite a lot. To solve this, we standardized the pictures to the size of 480x480, by padding the missing size with the edge values. The data was then represented as three stacked 480x480 matrices (one for each presentation of red, green and blue values in the picture, called *channels*) where each matrix contains values between 0 and 255. To avoid numerical problems in training our model, we also scaled the channels from the original scale of 0-255 to a scale between 0-1.
...
"sliding" a 3 x 3 convolution matrix (in our case) over the input array and produces a new *feature map*m i.e. a new representation of the input. 
...
The representation depends on both the inputted images and the values in the convolution matrix. 
...
Finally, a single output node connects to each of the previous fully connected 64 nodes. Linear equations are again followed by a nonlinear one, this time the sigmoid function, which outputs values between 0 and 1. Since the target is binary (edible or note), these values can be interpret as probabilities, and they can be used for predicting the class of the image. A natural choice is to predict a mushroom as eatable, if the probability of it being eatable is at least 0.5. This is not the only possible choice, however, as one might wish to be more conservative to avoid collecting poisonous mushrooms.
...
Since our model outputs a probability for edibility, the user can make a choice to only pick mushrooms that have a very high probability of edibility. The higher that treshold is, the less likely it is that the hunter will collect a poisonous or an inedible mushroom (high precision). However, the tradeoff is that the hunter is also more likely to leave some edible mushrooms in the forest (low recall).

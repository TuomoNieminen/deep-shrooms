...
Muhsrooms can be classified into several categories based on eatability. Reasons for not wanting to eat a mushroom include poor taste and poison. Correct classification of a found mushroom is a basic problem that a mushroom hunter faces.

- The hunter wishes to avoid inedible and poisonous mushrooms
- The hunter wishes to collect edible mushrooms

Further classification according to taste is also possible and in some cases poison can be removed by cooking. Humans are very good at categorizing items by appearance and other available information. Based on previous expert knowledge, the following information is useful for mushroom classification:
...
We used our classifier to build an interactive application which outputs the probability for edibility of inputted 480x480x3 images.
